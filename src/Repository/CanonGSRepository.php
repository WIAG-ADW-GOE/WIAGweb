...
class CanonGSRepository extends ServiceEntityRepository {
    ...
    */
    ...
    private function addQueryConditions(QueryBuilder $qb, CanonFormModel $formmodel): QueryBuilder {
        ...
        # identifier
        if($formmodel->someid) {
            $db_id = CanonGS::extractDbId($formmodel->someid);
    ...
    public function addMonasteryLocation(CanonGS $person) {
    ...
    public function findOneWithOffices($id) {
     ...
     public function findOfficeNames(CanonGSFormModel $canonquery) {
        $qb = $this->createQueryBuilder('canon')
